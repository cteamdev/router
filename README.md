...
1. Делаем реф на текущее состояние роутера:
```tsx
const refId = useRef(router.state.id);
```
2. Прокидываем текущее значение рефа в первый аргумент хука `useMeta`:
```tsx
const meta = useMeta(id.current);
```
